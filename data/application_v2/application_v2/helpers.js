...
Handlebars.registerHelper('formatdate2', function(apiDateStr) {
    if (apiDateStr != null && apiDateStr != '') {
        ...
        return `ចុះថ្ងៃទី${(apiDate.getDate() + '').padStart(2, '0')} ខែ${months[apiDate.getMonth()]} ឆ្នាំ${apiDate.getFullYear()}`;
    } else {
        return '';
    }
});
...
Handlebars.registerHelper('countItem', function(index) {
    return index + 1;
});
...
Handlebars.registerHelper('formatCurrency', function(number) {
 ...
 Handlebars.registerHelper('checkEmptyOrNull', function(number) {
    if (number === null || number === "" || number === 0) {
        return '<span>..........</span>'; // Return placeholder text with underline class
    } else {
        return '<span class="underline-text">' + Handlebars.helpers.formatCurrency(number) + '</span>'; // Format the number and wrap it with underline class
    }
});
...
Handlebars.registerHelper('equal_minus_one', function(a, b) {
    return a == b -1
});
...
Handlebars.registerHelper('EmptyOrNull', function (value, options) {
